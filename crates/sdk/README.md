...
- Avatar wrappers expose `total_balance`, aggregated trust helpers, `plan_direct_transfer` / `direct_transfer`, plan-first ERC20 wrapper helpers (`plan_wrap_demurrage_erc20`, `plan_wrap_inflation_erc20`, `plan_unwrap_demurrage_erc20`, `plan_unwrap_inflation_erc20`), and `plan_replenish` / `replenish`; human and organisation avatars also expose `max_replenishable` plus `plan_replenish_max` / `replenish_max`.
