use pathfinder::{create_flow_matrix, PathfinderError};
use alloy_primitives::U256;
use types::{FlowEdge, Stream};
...
fn test_create_flow_matrix_complex() {
    ...
    let expected_addresses = vec![
        sender, receiver, intermediate_a5, intermediate_63, token_owner_7b, token_owner_f7
    ];
...
fn test_create_flow_matrix_no_terminal_edges() {
    ...
    let transfers = vec![
        common::sample_transfer_step(sender, intermediate, sender, value)
    ];
    ...
    assert!(result.is_ok(), "Should succeed by making last edge terminal");
...
fn test_create_flow_matrix_multiple_terminal_edges() {
    ...
    assert!(result.is_ok(), "Should succeed with multiple terminal edges");
...
fn test_create_flow_matrix_source_coordinate() {
    ...
    let sender_index = matrix.flow_vertices.iter()
