use pathfinder::{find_path, PathfinderError};
...
async fn test_find_path() {
    ...
    if let Ok(transfers) = result {
        assert!(!transfers.is_empty(), "Should return at least one transfer step");
...
async fn test_find_path_with_zero_value() {
    ...
    match result {
        Ok(transfers) => {
            ...
            println!("Zero value request succeeded with {} transfers", transfers.len());
        }
        Err(e) => {
...
async fn test_find_path_same_sender_receiver() {
    ...
    match result {
        Ok(transfers) => {
            println!("Same address request succeeded with {} transfers", transfers.len());
        }
        Err(e) => {
