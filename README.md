...
| Area | Status | Notes |
| --- | --- | --- |
| `circles-rpc` | Good coverage | Core HTTP/query/event decoding is in place, including paged transaction history, the flexible cursor/order pagination needed for group membership, group queries, and group-holder views, plus invitation-origin / combined-invitation / inviter-outbound invitation helpers backed by the current Nethermind RPC surface. |
...
| `circles-sdk` | Partial, improving | Read flows and typed avatars are usable, replenish planning/execution rides the runner abstraction, and the convenience surface now covers aggregated trust helpers, direct-transfer planning/execution, transaction-history pagination, profile metadata / short-name writes, personal minting, max-replenish helpers, base-group read/write helpers, human group-membership detail helpers, top-level group member/collateral/holder reads, group-token mint/redeem/property helpers, invitation origin / inbound / outbound invitation queries, invitation fee/module/quota helpers, proxy-inviter discovery, invite-path lookup, farm-path lookup, deterministic referral-address computation, plan/execute direct invite, plan/execute batch referral generation, and the optional referrals backend client plus inviter-side public referral listing; the main remaining invitation gap is the higher-level referral-code storage/composition flow. |
...
The biggest remaining parity work is no longer the replenish planner, group convenience, invitation-query foundation, or the referrals service client. The open gaps are the last higher-level invitation / referral composition edges, especially referral-code storage/composition, and wallet-specific execution flows outside the generic runner abstraction.
