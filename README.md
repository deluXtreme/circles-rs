...
| `circles-sdk` | Partial, improving | Read flows and typed avatars are usable, replenish planning/execution rides the runner abstraction, the crate now ships built-in EOA and single-owner Safe runners for real write execution plus runner-level batch/gas-estimation/read-call helpers, and now also exposes `SafeExecutionBuilder` for canonical Safe payload/hash preparation ahead of browser or other external signing. The convenience surface covers aggregated trust helpers, consolidated profile-view / trust-summary / enriched trust / valid-inviter / enriched-transaction / unified profile-search reads, native cursor-based transaction-history / group-browse / group-membership / token-holder reads, derived profile-service URL handling, chain-RPC override support for direct contract calls, direct-transfer planning/execution, profile metadata / short-name writes, personal minting, max-replenish helpers, base-group read/write helpers, human group-membership detail helpers, top-level group member/collateral/holder reads, group-token mint/redeem/property helpers, invitation origin / inbound / outbound invitation queries, invitation fee/module/quota helpers, proxy-inviter discovery, invite-path lookup, farm-path lookup, deterministic referral-address computation, plan/execute direct invite, single-referral `getReferralCode` planning, plan/execute batch referral generation, and the optional referrals backend client plus inviter-side public referral listing; the remaining invitation work is now follow-up service/auth polish rather than missing core planner surface. |
...
The biggest remaining parity work is no longer the replenish planner, group convenience, invitation-query foundation, the core invitation/referral planners, the consolidated SDK-enablement RPC surface, or the latest cursor-native read/config alignment follow-up. The open gaps are the remaining service/auth polish around referrals, the per-source invitation helper follow-up from the latest TS/RPC updates, plus the broader wallet/backend surface beyond the current EOA and single-owner Safe runners. Browser-backed Safe preparation is now in place, but actual browser-provider signing/submission is still a follow-up.
